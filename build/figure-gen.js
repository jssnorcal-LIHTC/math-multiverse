...
const {
  VB_W, VB_H, INK, GRID, PLOT_GRID, DEFAULT_ACCENT,
  LABEL_FONT, TICK_FONT, NOTE_FONT,
  n2, esc, hp, estimateTextWidth, svgRootLines,
...
// ---- fixed geometry ----
// VB_W, VB_H and estimateTextWidth now come from build/figure-tokens.js (destructured above).
...
// estimateTextWidth (a per-character Times New Roman table since Stage D, 26-0921;  a flat 0.6 em
// before it) is shared by the left-margin sizing below, the text-bbox assertions in
// tests/figure-derive.js and tests/figure-fidelity.js AND the four document renderers, so it lives
// in build/figure-tokens.js and none of them can disagree about how wide a label is.
...
// estimateTextWidth moved to build/figure-tokens.js, where it reads the committed per-character table.
...
function genSvgPanels(dataTable, accentColor) {
  ...
  const out = [];
  // The shared root (build/figure-tokens.js), not a local copy:  it now carries the font family the
  // width table was measured from, and two hand-kept copies are how a chart root would drift.
  out.push(...svgRootLines());
...
module.exports = {
  ...
  GEN_KINDS, DOC_TYPES,
  INK, GRID, PLOT_GRID, DEFAULT_ACCENT, estimateTextWidth, VB_W, VB_H, MAX_NOTES, MIN_PLOT_H,
