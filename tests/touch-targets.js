...
const PROBE = (minPx) => {
  ...
  const rows = controls.map((el) => { const d = describe(el); const r = el.getBoundingClientRect(); d.top = Math.round(r.top); d.left = Math.round(r.left); return d; });
...
(async () => {
  ...
  for (const r of rows) {
    ...
    console.log(`  ${String(r.id).padEnd(14)} at ${String(r.left).padStart(4)},${String(r.top).padStart(3)}  painted ${String(r.painted.w).padStart(3)}x${String(r.painted.h).padStart(2)}   effective hit ${String(r.effective.w).padStart(5)}x${String(r.effective.h).padStart(5)}   ${ok ? 'OK' : 'SHORT'}`);
    if (!ok) {
      const m = r.misses[0];
