...
function evalModule(body, id, prelude) {
  const epilogue = `
...
function loadModules() {
  ...
  for (const m of MODULES) {
    out[m.id] = evalModule(sliceBody(src, m.marker), m.id, prelude);
  }
  return out;
}
...
function buildDrivers(mods) {
  ...
  const rk = mods['rocky-translator'];
