...
// the wrong picture).
//
// FIX ROUND 5 (this file, roundTripCheck() in Part 2 below): FIX ROUND 3 above hoisted the round-
// trip check's write to the committed fixture (tests/fixtures/vis-demo/f-chart.svg) out from behind
// the arming guard, which made that write unconditional -- and unconditional meant a genuine
// failure could self-heal on the very next run, because the write silently overwrote the committed
// fixture with whatever the (possibly wrong) regenerated bytes turned out to be. roundTripCheck()
// now snapshots the fixture's bytes before the real write, still exercises the real CLI write path
// (proving that path actually repairs a red gate is the entire point of the check), and restores
// the original bytes itself once its assertion has run, on every exit path. See the comments at
// roundTripCheck() and at its call site for the full finding.
//
...
// re-derives. This WRITES tests/fixtures/vis-demo/f-chart.svg for real, through the real path --
// that write is the entire point, since a dry run would only prove the comparison, not that
// "regenerate" actually repairs a red gate.
//
// FIX ROUND 5: an unconditional write to a COMMITTED oracle file lets a genuine failure self-heal
// on re-run -- if the generator drifts, this write would silently overwrite the fixture with the
// (wrong) new bytes, so the very next invocation byte-compares the fixture against itself and
// reports clean. So this check snapshots the fixture's bytes first, lets the real write happen,
// makes its assertion, and restores the original bytes afterward in a `finally` -- on every exit
// path, including a refusal, a failed assertion, or a thrown error -- so the working tree is left
// exactly as found regardless of outcome and a failing run cannot launder the fixture it just
// proved wrong.
function roundTripCheck() {
  const { pack, fig } = loadFixture();
  const srcAbs = path.join(REPO_ROOT, fig.src);
  const existedBefore = fs.existsSync(srcAbs);
  const original = existedBefore ? fs.readFileSync(srcAbs, 'utf8') : null;
  try {
    ...
    // Restore exactly what was there before this check ran, whether the write changed it, the
    // assertion above failed, or something threw -- a real oracle file must never come out of a
    // test run holding unreviewed bytes.
    if (existedBefore) {
      const after = fs.existsSync(srcAbs) ? fs.readFileSync(srcAbs, 'utf8') : null;
      if (after !== original) fs.writeFileSync(srcAbs, original, 'utf8');
...
// write to that file is REAL, not a dry run: it goes through the same regenerate() CLI path
// `node build/figure-gen.js` uses with no --outdir, because proving that path actually repairs a
// red gate is the entire point (see FIX ROUND 5 at roundTripCheck() itself). roundTripCheck()
// snapshots the committed bytes before that write and restores them itself once its assertion has
// run, on every exit path, so this call site does not need to make the write safe. An earlier
// version of this comment called the write "a verified no-op regardless of arming" -- true only
// while the committed fixture already matched the generator's output, which is exactly the fact
// the round-trip check exists to test, so that argument assumed its own conclusion: left
// unconditional and unrestored, the write let a genuine failure self-heal on the very next
// invocation by silently overwriting the committed fixture with the (wrong) regenerated bytes, so
// the same broken dataTable then byte-matched and the gate reported clean.
try {
